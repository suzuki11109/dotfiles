...
(use-package exec-path-from-shell
  :if (or (memq window-system '(ns x))
          (daemonp))
  :init
  (setq exec-path-from-shell-arguments '("-l"))
  (setq exec-path-from-shell-variables '("PATH" "MANPATH" "JAVA_HOME" "KUBECONFIG" "NPM_TOKEN" "PNPM_HOME"))
...
(use-package emacs
  ...
  (after-init . (lambda ()
                         ...
                         (set-face-attribute 'mode-line nil
                                             :family (face-attribute 'variable-pitch :family) :height 1.0)
...
(setq mode-line-right-align-edge 'right-fringe) ;; prevent aligning with visual-fill-column-mode
...
;;   :config
;;   ;; global-auto-revert-mode can slow for non-file buffers. try to enable it per active window.
...
(use-package tabspaces
  ...
  (define-key tabspaces-command-map (kbd "m") 'tab-move)
...
(use-package vertico
  ...
  (vertico-multiform-commands
   '((consult-ripgrep buffer)
     (compile-pro-compile-from-history (vertico-sort-function . nil))))
...
(use-package org-modern
  ...
  (let ((todo-bg (face-foreground 'org-todo nil t))
        ...
        (fg (face-background 'default nil t)))
...
(use-package jtsx
  ...
  :preface
  (defun +jtsx-evilnc-comment (beg end)
    "Wrapper to make `jtsx-comment-dwim' work with `evil-nerd-commenter'."
    (let ((mark-active t))
      (save-excursion
        (goto-char beg)
        (set-mark (point))
...
(use-package dockerfile-ts-mode
  ...
  :preface
  (defun +dockerfile-ts-mode-setup ()
    (add-hook 'before-save-hook 'lsp-format-buffer nil t))
  :hook
...
(use-package fish-completion
  ...
  (minibuffer-setup . (lambda ()
                        (when (memq this-command '(compile compile-pro-compile))
...
(use-package eca
  ...
  (eca-chat-mode . (lambda ()
                     (evil-insert-state)))
  :custom
  (eca-chat-expand-pending-approval-tools nil)
  ...
  (defun eca-chat--mode-line-spacer-image ()
    ...
    (let* ((height (max (or doom-modeline-height 25)
                        (frame-char-height)))
           (bg (face-attribute 'mode-line :background nil 'default)))
      (create-image
       (concat "P1\n1 " (number-to-string height) "\n"
               (mapconcat (lambda (_) "1")
                          (make-list height nil) "\n"))
       'pbm t :background bg :foreground bg :ascent 'center)))
  ...
  (defun eca-chat--mode-line-string (session)
    ...
    (if eca-chat--closed
      ...
      (let* ((fmt eca-chat-mode-line-format)
             ...
             ;; Sync height to doom-modeline
             (height (max (or doom-modeline-height 25)
                          (frame-char-height)))
             )
        (let ((result (concat (propertize " " 'display (eca-chat--mode-line-spacer-image)) " " left fill right)))
